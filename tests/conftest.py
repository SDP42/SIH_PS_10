...
# app/fhir_extra.py each hardcode DB_PATH = "db/ayush_icd11_combined.db" as a
# module-level constant (no settings object to override via env var), so the
# only reliable interception point is patching that attribute directly on
# each already-imported module before any test runs.
...
for _mod in (_api, _conceptmap, _ai_mapping, _governance, _fhir_extra):
